...
export const platformFormSchema = z.object({
  ...
  service_filters_text: z.string().optional(),
...
export function platformToFormValues(platform: Platform): PlatformFormValues {
  ...
  const serviceFilters = Array.isArray(platform.service_filters) ? platform.service_filters : [];
  ...
  return {
    ...
    service_filters_text: serviceFilters.join("\n"),
...
function toPlatformPayloadBase(values: PlatformFormValues) {
  return {
    ...
    service_filters: parseLinesToList(values.service_filters_text, (value) => value.toLowerCase()),
