param(
    [string]$TargetHost = $(if ($env:SMOKE_SSH_HOST) { $env:SMOKE_SSH_HOST } else { '107.148.164.86' }),
    ...
    [string]$RemotePath = $(if ($env:SMOKE_REMOTE_PATH) { $env:SMOKE_REMOTE_PATH } else { '/opt/Resin' }),
